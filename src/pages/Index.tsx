...
const Index = () => {
  ...
  return (
    <div className={`min-h-screen bg-background transition-colors duration-300 cursor-none ${isDarkMode ? 'dark' : ''}`}>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="hidden md:flex items-center space-x-6">
              {[
                ...
                <button
                  key={item.id}
                  className={`text-foreground hover:text-barber-terracotta transition-colors relative group cursor-pointer ${
                    activeSection === item.id ? 'text-barber-terracotta' : ''
                  }`}
                  onClick={() => scrollToSection(item.id)}
                >
                  {item.name}
                  <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-barber-terracotta transition-all group-hover:w-full"></span>
                </button>
              ))}
              <Button
                variant="outline"
          ...
          {isMenuOpen && (
            <div className="md:hidden mt-4 pb-4 border-t border-border animate-fade-in">
              {[
                ...
                <button
                  key={item.id}
                  className="block w-full text-left py-2 text-foreground hover:text-barber-terracotta transition-colors cursor-pointer"
                  onClick={() => scrollToSection(item.id)}
                >
                  {item.name}
                </button>
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="text-center animate-fade-in">
            <h1 className="text-4xl md:text-6xl font-bold text-white mb-6">
              Traditional barber
              <span className="text-barber-terracotta block">Hair Artistry</span>
            </h1>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
              <Button 
                size="lg" 
                className="bg-barber-terracotta hover:bg-barber-desert text-white px-8 py-3 cursor-pointer"
              ...
              <Button 
                ...
                className="border-white text-white hover:bg-white hover:text-barber-terracotta cursor-pointer"
              >
                📄 Download Price List
              </Button>
            </div>
          ...
          {heroSlides.map((_, index) => (
            <button
              ...
              className={`w-3 h-3 rounded-full transition-colors cursor-pointer ${
                index === currentSlide ? 'bg-barber-terracotta' : 'bg-white/50'
              }`}
            />
          ))}
        </div>
            ...
            <div className="animate-slide-in-left">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center">
                  <div className="text-2xl font-bold text-barber-terracotta">2500+</div>
                  <div className="text-sm text-muted-foreground">Happy Clients</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-barber-terracotta">25</div>
              ...
              <Card key={index} className="hover-lift animate-scale-in cursor-pointer" style={{animationDelay: `${index * 0.1}s`}}>
                <CardContent className="p-0">
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center justify-between">
                      <Badge variant="secondary" className="bg-barber-sand text-barber-desert">
                        {service.price}
                      </Badge>
            ...
            {products.map((category, index) => (
              <Card key={index} className="hover-lift animate-fade-in" style={{animationDelay: `${index * 0.2}s`}}>
                <CardContent className="p-6">
                  <h3 className="text-xl font-semibold mb-4 text-barber-terracotta">{category.category}</h3>
                  <ul className="space-y-2">
                    {category.items.map((item, itemIndex) => (
                      <li key={itemIndex} className="flex items-center">
                        <Check className="h-4 w-4 text-barber-terracotta mr-2" />
      ...
      <section id="cultural" className="py-16 barber-pattern">
        
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold mb-4">Our Cultural Roots</h2>
            <p className="text-muted-foreground max-w-2xl mx-auto">
              The barber people have a rich tradition of hairdressing that dates back centuries, 
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            <div className="animate-slide-in-left">
              ...
              <div className="mt-8">
                ...
                <Button 
                  variant="outline" 
                  className="w-full border-barber-terracotta text-barber-terracotta hover:bg-barber-terracotta hover:text-white cursor-pointer"
                  onClick={bookAppointment}
                >
                ...
                <div className="flex space-x-4">
                  {socialLinks.map((social, index) => (
                    <Button
                      ...
                      className="hover:bg-barber-terracotta hover:text-white hover:border-barber-terracotta cursor-pointer"
                    >
              ...
              <div className="bg-barber-sand/10 p-6 rounded-lg">
                ...
                <div className="bg-barber-terracotta/10 p-4 rounded border border-barber-terracotta/20">
                  <p className="text-sm text-center">
                    "Experience the authentic barber hairdressing tradition"
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="flex justify-center space-x-4">
            {socialLinks.map((social, index) => (
              <Button
                ...
                size="sm"
                onClick={() => window.open(social.url, '_blank')}
                className="text-barber-sand hover:text-white hover:bg-barber-terracotta cursor-pointer"
              >
                <social.icon className="h-4 w-4" />
              </Button>
            ))}
